...
.scene { position:relative; width:100vw; height:100svh; }
...
/* SHRINE */
.shrine { position:absolute; bottom:18%; left:50%; transform:translateX(-50%); display:flex; flex-direction:column; align-items:center; cursor:pointer; z-index:5; transition:filter .25s; touch-action:manipulation; }
.shrine::before { content:''; position:absolute; inset:-24px; border-radius:20px; z-index:-1; }
...
/* NPC */
.npc { position:absolute; bottom:17%; display:flex; flex-direction:column; align-items:center; animation:npc-idle var(--speed) ease-in-out infinite; cursor:pointer; z-index:4; touch-action:manipulation; }
.npc::before { content:''; position:absolute; inset:-28px -22px; border-radius:16px; }
...
/* HADA */
.hada { position:absolute; top:6%; left:50%; transform:translateX(-50%); display:flex; flex-direction:column; align-items:center; animation:hada-float 5.5s ease-in-out infinite; cursor:pointer; z-index:4; }
...
/* WHISPER ALERT */
.whisper-alert { display:none; position:absolute; top:17%; left:50%; transform:translateX(-50%); background:rgba(14,10,28,.92); border:1px solid rgba(200,230,255,.35); color:rgba(220,240,255,.88); font-size:11px; padding:7px 16px; border-radius:4px; letter-spacing:.5px; white-space:nowrap; box-shadow:0 0 24px rgba(140,200,255,.25); z-index:15; animation:whisper-in .6s ease-out; max-width:90vw; white-space:normal; text-align:center; }
